    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['tps:attachment:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['tps:attachment:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['tps:attachment:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="attachmentList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['tps:attachment:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['tps:attachment:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
import { listAttachment, getAttachment, delAttachment, addAttachment, updateAttachment } from "@/api/tps/attachment"
...
function handleSelectionChange(selection) {
  ids.value = selection.map(item => item.attachmentId)
...
function submitForm() {
  proxy.$refs["attachmentRef"].validate(valid => {
    if (valid) {
      if (form.value.attachmentId != null) {
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除任务附件编号为"' + _attachmentIds + '"的数据项？').then(function() {
...
function handleExport() {
  proxy.download('tps/attachment/export', {
    ...queryParams.value
